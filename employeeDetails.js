...
function findEmployeeById(employeeId) {
 ...
 function displayEmployees() {
    const EmployeesDisplay = employees.map((employee, index) => `<p>${employee.id}: ${employee.name}: ${employee.name} - ${employee.department} - $${employee.salary}- ${foundEmployee.specialization}</p>`).join('');
